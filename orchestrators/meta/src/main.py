...
if env_path.exists():
    ...
    print(f"[META] Loaded .env from {env_path}")
...
# LLM for orchestrator responses - Priority: Gemini > DeepSeek > OpenRouter > Gateway > OpenAI > Anthropic
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY", "")
DEEPSEEK_API_KEY = os.getenv("DEEPSEEK_API_KEY", "")
OPENROUTER_API_KEY = os.getenv("OPENROUTER_API_KEY", "")
GATEWAY_URL = os.getenv("IAFACTORY_GATEWAY_URL", "http://localhost:3001")
...
async def generate_agent_response(
    ...
    """Génère la réponse de l'agent via LLM (Gateway first, then direct APIs)"""
    import httpx
    ...
    # 1. Try Gemini first (free tier available)
    if GEMINI_API_KEY:
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
    ...
    # 2. Try OpenRouter (fallback - many models)
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.post(
                    ...
                    headers={
                        ...
                        "X-Title": "IA Factory Algeria Nexus"
                    },
                    json={
    ...
    # 2. Fallback: Try IAFactory Gateway
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.post(
                ...
                headers={
                    "Content-Type": "application/json",
                    "Authorization": "Bearer nexus-internal-service"
                },
                json={
                    "model": "deepseek-chat",
                    "messages": [
    ...
    # 2. Fallback: Try OpenAI directly
    if OPENAI_API_KEY:
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.post(
    ...
    # 3. Fallback: Try Anthropic directly
    if ANTHROPIC_API_KEY:
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.post(
    ...
    # 4. Last resort: greeting from agent in the correct language
    print(f"[WARN] All LLM calls failed, using fallback greeting for {agent.value}")
    persona = AGENT_PERSONAS.get(agent, {})
    greeting = persona.get("greeting", {})
    if isinstance(greeting, dict):
        return greeting.get(lang, greeting.get("fr", f"Salut! Je suis {agent.value}. Comment puis-je t'aider?"))
    return greeting if isinstance(greeting, str) else f"Salut! Je suis {agent.value}. Comment puis-je t'aider?"
