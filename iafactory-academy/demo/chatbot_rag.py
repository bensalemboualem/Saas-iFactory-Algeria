...
def generate_simple_response(query: str, context: str, language: str = "fr") -> str:
    ...
    if not context.strip():
        responses = {
            ...
            "ar": "لم أجد معلومات ذات صلة في قاعدة معرفتي للإجابة على هذا السؤال.",
    ...
    intros = {
        ...
        "ar": "إليك ما وجدته في قاعدة معرفتي:",
...
Cite tes sources entre crochets [Source X].""",
        ...
        "ar": """أنت مساعد ذكاء اصطناعي خبير في IAFactory-School، البرنامج الوطني للذكاء الاصطناعي للتعليم في الجزائر.

تجيب على الأسئلة بناءً على السياق المقدم فقط.
إذا لم تكن المعلومات موجودة في السياق، قل ذلك بوضوح.
كن دقيقًا في الأرقام (الأسعار، العائد على الاستثمار، المدد).""",
...
def render_chat_message(role: str, content: str, sources: List[Dict] = None):
    ...
    if sources and role == "assistant":
        with st.expander("📚 Sources citees", expanded=False):
...
def main():
    ...
    with st.sidebar:
        ...
        language = st.selectbox(
            ...
            options=["fr", "ar", "en"],
            format_func=lambda x: {"fr": "Francais", "ar": "العربية", "en": "English"}[x],
            index=0
        )
    ...
    st.markdown(f"""
        <div class="main-header">
            <h1>🤖 Assistant IA - {config['display_name']}</h1>
    ...
    placeholders = {
        ...
        "ar": "اطرح سؤالك حول IAFactory-School...",
    ...
    if not st.session_state.messages:
        ...
        examples = {
            "fr": [
                "Quel est le prix par élève pour une école de 1600 élèves?",
                "Quel est l'investissement total IAFactory pour BBC School?",
                "Quelles technologies sont utilisées dans la plateforme?",
                "Quel est le ROI prévu sur 3 ans?",
            ],
            "ar": [
                "ما هو سعر كل طالب؟",
                "ما هو إجمالي الاستثمار؟",
