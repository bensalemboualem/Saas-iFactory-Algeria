            ...
            <div class="row">
                <div class="col-xl-7 col-lg-6 col-md-6">
                    <div class="statement_info mb_30">
                        <h3>{{ @$sections['statement']->name ?? @$sections['statement']->defaultTranslate->name }}</h3>
                        <ul class="statement_lists">
                            {{-- @dd($sections['statement']->data) --}}
                            @foreach (is_array(@$sections['statement']->data ?? @$sections['statement']->defaultTranslate->data ?? []) ? (@$sections['statement']->data ?? @$sections['statement']->defaultTranslate->data ?? []) : [] as $item)
                                <li>
            ...
            <div class="row justify-content-center">
                <div class="col-xl-6">
                    <div class="section__title white_text text-center">
                        <h3>{{ @$sections['study_at']->name ?? @$sections['study_at']->defaultTranslate->name }}</h3>
                        <p>{{ @$sections['study_at']->description ?? @$sections['study_at']->defaultTranslate->description }}</p>
                    </div>
                </div>
            </div>
            <div class="row">
                @foreach (is_array(@$sections['study_at']->data ?? @$sections['study_at']->defaultTranslate->data ?? []) ? (@$sections['study_at']->data ?? @$sections['study_at']->defaultTranslate->data ?? []) : [] as $key => $item)
                    <div class="col-xl-4 col-md-4">
                        <div class="serevice_box mb_30">
                            <div class="icon">
                                @php
                                    $org = $sections['study_at']->data ?? [];
                                @endphp
                                <img src="{{ @globalAsset(uploadPath(@$org[$key]['icon']), '90X60.webp') }}"
                                    alt="Icon">
                            </div>
                            <h3>{{ $item['title'] }}</h3>
                            <p>{{ $item['description'] }}</p>
                    ...
                    @foreach ($data['abouts'] as $key => $item)
                        @if ($key % 2 == 0)
                            <div class="single_about_gallery">
                                ...
                                <div class="single_about_content">
                                    <div class="iconImg">
                                        <img src="{{ @globalAsset(@$item->icon_upload->path, '90X60.webp') }}"
                                            alt="Image" class="img-fluid">
                                    </div>
                        ...
                        @else
                            <div class="single_about_gallery">
                                <div class="single_about_content">
                                    <div class="iconImg">
                                        <img src="{{ @globalAsset(@$item->icon_upload->path, '65X90.webp') }}"
                                            alt="Image" class="img-fluid">
                                    </div>
                                    <h4>{{ @$item->defaultTranslate->name }}</h4>
                                    <p>{{ @$item->defaultTranslate->description }}</p>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="section__title mb_76 text-center">
                        <h3 class="text-capitalize">{{ @$sections['our_teachers']->name ?? @$sections['our_teachers']->defaultTranslate->name }}</span></h3>
                    </div>
                </div>
            </div>
            <div class="row">
